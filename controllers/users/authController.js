...
redisClient.connect()
  ...
  .catch(err => {
    console.warn('⚠️ Redis connection failed. Falling back to in-memory OTP store.');
    // Don't log the whole error to keep terminal clean, just the message
...
export const sendOtp = async (req, res) => {
  ...
  try {
    ...
    if (redisConnected) {
      // Store OTP with expiry (5 min) in Redis
      ...
      console.log(`[Memory OTP] Stored OTP for ${mobile}: ${otp}`);
    }
...
export const verifyOtp = async (req, res) => {
  ...
  try {
    ...
    let storedOtp;
    if (redisConnected) {
      storedOtp = await redisClient.get(`otp:${mobile}`);
      if (storedOtp === otp) {
      ...
      if (entry && entry.expiresAt > Date.now()) {
        storedOtp = entry.otp;
        if (storedOtp === otp) {
          delete otpStore[mobile]; // Clean up
        }
      }
    }
